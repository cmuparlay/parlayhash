...
struct scheduler_info {
  ...
  extern inline scheduler_info& get_global_scheduler_info() {
    static scheduler_info global_default_sched;
    return global_default_sched;
  }
...
struct scheduler;
  ...
  struct workerInfo {
    struct global_id {
      int id;
      global_id(int id) : id(id) {} 
      global_id() : id(-1) {}
      ~global_id() {
	if (id != -1)
	  get_global_scheduler_info().remove_worker(id);
      }
    };
    int local_worker_id;
    global_id global_worker;
    Scheduler* my_scheduler;
    workerInfo() : local_worker_id(-1), global_worker(-1), my_scheduler(nullptr) {}
    workerInfo(int localid, Scheduler* s) :
      local_worker_id(localid),
      global_worker(get_global_scheduler_info().add_worker()),
      my_scheduler(s) {}
  };
