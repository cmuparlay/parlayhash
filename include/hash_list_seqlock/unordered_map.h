...
struct unordered_map {
  ...
  struct Table {
    ...
    size_t size;
    big_atomic<link>* get_bucket(const K& k) {
      size_t idx = Hash{}(k) & (size - 1u);
      return &table[idx].v;
    }
    explicit Table(size_t n) {
      unsigned bits = parlay::log2_up(n);
      size = 1ul << bits;
  ...
  static link* new_link(const K& k, const V& v, link* l) { return epoch::New<link>(k, v, l); }

  explicit unordered_map(size_t n) : hash_table(Table(n)) {}
  ~unordered_map() {
    auto& table = hash_table.table;
    parlay::parallel_for(0, table.size(), [&](size_t i) { retire_list_all(table[i].v.load().next); });
  }
  ...
  bool insert(const K& k, const V& v) {
    ...
    auto [l, tg] = s->ll();
    if (is_empty(l.next))
      if (s->sc(tg, link(k, v, nullptr))) return true;
    return epoch::with_epoch([&] {
      ...
      while (true) {
        ...
        if (is_empty(l.next)) {
          ...
          if (s->sc(tg, link(k, v, new_head))) return true;
          epoch::Delete(new_head);
        }
        if (cnt++ > 1000000) {
          std::cout << "insert infinite loop?" << std::endl;
          abort();
        }
      }
    });
  ...
  bool remove(const K& k) {
    ...
    return epoch::with_epoch([&] {
      ...
      while (true) {
        ...
        if (KeyEqual{}(l.key, k)) {
          if (l.next == nullptr) {
            if (s->sc(tg, link())) return true;
          } else {
            if (s->sc(tg, link(l.next->key, l.next->value, l.next->next))) {
              epoch::Retire(l.next);
              return true;
            }
          }
          ...
          if (s->sc(tg, link(l.key, l.value, new_head))) {
        ...
        if (cnt++ > 1000000) {
          std::cout << "remove infinite loop?" << std::endl;
          abort();
        }
      }
    });
  ...
  long size() {
    return parlay::reduce(parlay::map(hash_table.table, [&](bucket& x) { return list_length(x.v.load()); }));
  }
};
