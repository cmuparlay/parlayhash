...
class HazardPointers {
  ...
  struct RetiredList {
    ...
    void push(garbage_type* p) noexcept { p->set_next(std::exchange(head, p)); }

    void swap(RetiredList& other) { std::swap(head, other.head); }
  ...
  struct alignas(CACHE_LINE_ALIGNMENT) HazardSlot {
    ...
    std::atomic<garbage_type*> protected_ptr_2{nullptr};
  ...
  void relinquish_slot(HazardSlot* slot) { slot->in_use.store(false); }
  ...
  struct HazardSlotOwner {
    ...
    ~HazardSlotOwner() { list.relinquish_slot(my_slot); }
 ...
 public:
  ...
  void protect_direct(garbage_type* src) {
    local_slot.my_slot->protected_ptr_2.exchange(src, std::memory_order_seq_cst);
  }

  void unprotect_direct(garbage_type* src) {
    local_slot.my_slot->protected_ptr_2.exchange(nullptr, std::memory_order_seq_cst);
  }
  ...
  void release() { local_slot.my_slot->protected_ptr.store(nullptr, std::memory_order_release); }
 ...
 private:
  ...
  void scan_hazard_pointers(F&& f) noexcept(std::is_nothrow_invocable_v<F&, garbage_type*>) {
    for_each_slot([&, f = std::forward<F>(f)](HazardSlot& slot) {
      ...
      auto p2 = slot.protected_ptr_2.load();
      if (p2) {
        f(p2);
      }
    });
