...
private:
  ...
  static std::optional<bool> try_remove_at(table_version* t, bucket* s, const K& k) {
    ...
    assert(old_node != nullptr && old_node->find_index(k) != -1);
    node* new_node = remove_from_node(old_node, k);
    if(s->compare_exchange_strong(old_node, new_node)) {
      retire_node(old_node);
      return true;
    } 
    destruct_node(new_node);
