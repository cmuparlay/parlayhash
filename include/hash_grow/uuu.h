...
struct unordered_map {
  ...
  struct prim_table {
	...
	block_status(parlay::tabulate<std::atomic<status>>(t->size/block_size, [&] (long i) {return Empty;}, size)) {}
  };
  ...
  void insert(prim_table* t, KV& key_value) {
    ...
    if (is_tagged(x)) std::cout << "dest is tagged during copy: " << x << std::endl;
    assert(!is_tagged(x));
  ...
  void remove(prim_table* t, KV& key_value) {
    size_t idx = t->get_index(key_value.key);
    node* x = t->buckets[idx].load();
    if (is_tagged(x)) std::cout << "dest is tagged during backout removal: " << x << std::endl;
    assert(!is_tagged(x));
  ...
  std::optional<bool> try_insert_at_(prim_table* t, long i, const K& k, const V& v, bool upsert) {
    node* x = t->buckets[i].load();
    // if (is_tagged(x)) {
    //   prim_table* nxt = t->next.load();
    //   if (nxt == nullptr) std::cout << "bad next: " << x << std::endl;
  ...
  bool insert(const K& k, const V& v) {
    prim_table* ht = hash_table.load();
    slot* s = &ht->buckets[ht->get_index(k)];
    __builtin_prefetch (s);
    return flck::with_epoch([&] {
  ...
  bool upsert(const K& k, const V& v) {
    ...
    return flck::with_epoch([&] {
      return flck::try_loop([&] {
	  copy_if_needed(idx);
          return try_insert_at(ht, idx, k, v, true);});});
  }
