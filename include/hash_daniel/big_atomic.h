...
#include "parlay/internal/concurrency/acquire_retire.h"
#include "parlay/internal/concurrency/marked_ptr.h"
...
struct big_atomic {
 ...
 public:
  ...
  big_atomic(const T& t) : version(0), indirect_value(nullptr), fast_value{} {
    // force correct static initialization order
    allocator().init();
    hazptr_instance();
  ...
  bool compare_and_swap(const T& expected, const T& desired) {
    ...
    if (!KeyEqual{}(expected, current)) {
      return false;
    }
    else if (KeyEqual{}(current,expected) && KeyEqual{}(expected, desired)) {
      return true;
    }
