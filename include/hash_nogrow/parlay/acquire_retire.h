...
#ifndef PARLAY_ALLOCATORS_ACQUIRE_RETIRE_H
#define PARLAY_ALLOCATORS_ACQUIRE_RETIRE_H
...
class acquire_retire {
 ...
 public:
  explicit acquire_retire(Deleter deleter_ = {}) :
    num_threads_and_deleter(thread_id.max_id,std::move(deleter_)),
    announcements(std::make_unique<padded<std::atomic<T*>>[]>(get_max_num_threads())),
    in_progress(std::make_unique<padded<bool>[]>(get_max_num_threads())),
    retired(std::make_unique<padded<std::vector<T*>>[]>(get_max_num_threads())),
    amortized_work(std::make_unique<padded<size_t>[]>(get_max_num_threads())) {}
  ...
  ~acquire_retire() {
    for (size_t i = 0; i < get_max_num_threads(); i++) {
      in_progress[i] = true;
    }
    ...
    // so on recursively.
    while (std::any_of(&retired[0], &retired[0] + get_max_num_threads(),
 ...
 private:
  ...
  void scan_slots(F &&f) {
    ...
    for (size_t i = 0; i < get_current_num_threads(); i++) {
  ...
  void work_toward_ejects(size_t work = 1) {
    ...
    auto threshold = std::max<size_t>(30, delay * get_current_num_threads());  // Always attempt at least 30 ejects
  ...
  size_t get_max_num_threads() const noexcept {
    return std::get<0>(num_threads_and_deleter);
  }

  size_t get_current_num_threads() const noexcept {
    return thread_id.max_current_id();
  }
...
#endif  // PARLAY_ALLOCATORs_ACQUIRE_RETIRE_H
